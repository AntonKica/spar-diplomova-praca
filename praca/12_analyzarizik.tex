...
Analýza rizík je proces, ktorým identifikujeme a vyhodnocujeme riziká voči aktívam organizácie\footnote{Zníženie rizika spadá do správy
rizík, čo je oddelený proces.}. Podľa \cite{BSI3} tento proces pozostáva zo štyroch hlavných krokov a niekoľkých podkrokov (viď
\ref{fig:risk-analysis-bsi3}). V časti \ref{sec:bsiphilosophy} sme naznačili, že BSI sa snaží \say{odbremeniť} malé a stredne veľké
organizácie, čo môžeme vidieť aj v tom, ako postupovali pri analýze rizík: rozdelili ju na dve časti, a to implicitnú a explicitnú.

\input tikz/risk-analysis-bsi3.tex

\textbf{Implicitná} časť analýzy rizík je tá, ktorú za nás vykonal BSI a je súčasťou Kompendia IT-Grundschutz \cite{GSK}. BSI vybral
najčastejšie\footnote{Podľa neoficiálnych odhadov je to 80\% spoločných aktív, zvyšných 20\% tvoria aktíva špecifické pre konkrétnu organizáciu.}
aktíva, ktoré používa väčšina organizácii a pre ne vykonal analýzu rizík. Všetky potrebné informácie (popis, cieľ, rozsah, opatrenia) sú
...
V nasledujúcich častiach práce budeme pod pojmom analýza rizík chápať \emph{explictnú analýzu rizík}, pokiaľ explicitne neuvedieme, že 
odkazujeme na \emph{implicitnú analýzu rizík}.

V minulej kapitole (časť \emph{\ref{sec:isms} -- \nameref{sec:isms}}) sme popísali kroky vedúce k analýze rizík, ktoré za pomoci \cite{BSI2} vieme zhrnúť na:
\begin{itemize}[nosep]
    ...
    \item posúdenie bezpečnostných požiadaviek aktív
...
si zadefinujeme modelovú situáciu. Upozorňujeme čitateľa, že realitu a detaily mierne skreslíme pre jednoduchosť ilustrácie. Uvedomujeme si potenciálnu
...
pečenia naprieč Slovenskom. Nakoľko NOIP je štátna inštitúcia, vzťahujú sa na ňu relevantné zákony KIB (spomínane v úvode práce), a preto za pomoci 
manažéra KIB sme iniciovali bezpečnostný proces.

NOIP je moderná organizácia a jej zamestnanci sú vybavení pracovnými notebookmi, pomocou ktorých vykonávajú svoje pracovné povinnosti, ako napríklad
komunikácia s kolegami, organizátormi a súťažiacimi. Počas analýzy procesov a bezpečnostných požiadaviek sme dospeli k tomu, že notebooky sú kľúčovým
aktívom pre zamestnancov. Špecifická dynamika organizovania olympiád (cestovanie a prítomnosť na podujatiach) a administratíva (spracovanie osobných
údajov a utajenie úloh) nás prinútila toto aktívum podrobiť explicitnej analýze rizík. Pre notebooky sme vo fáze modelovania použili modul \cite{GSK}
\emph{SYS.3.1 Laptops}.

\subsection{Určenie relevantných hrozieb}
Pri určovaní relevantných hrozieb vychádzame z modulu, na ktorý sme aktívum namapovali. Postupujeme v troch krokoch, najprv z katalógu \cite{GSK}
pre relevantný modul zozbierame identifikované základné hrozby, potom posúdime relevantnosť základných hrozieb z katalógu a identifikujeme
dodatočné špecifické hrozby. Pri určovaní relevantných hrozieb voči aktívu vychádzame z modulu, ktorý je najbližší analyzovanému aktívu (v našom
prípade to je modul \emph{SYS.3.1 Laptops}.)

Relevantnosť základnej hrozby vyjadrujeme v troch mierach \emph{irelevantná, nepriamo relevantná a relevantná} (pozri vysvetlenie \ref{tab:elemthreatrel}).
\begin{table}[h!]
...
\subsection{Klasifikácia rizík}

Pre identifkované hrozby stanovujeme hodnoty rizika, pričom hodnotu rizika vyjadrujeme ako strednú hodnotu dopadu hrozby, t.j.
$riziko = pravdepodobnosť * dopad hrozby$ \cite{MFSRIB}, neexistuje univerzálny spôsob, ako určiť hodnotu týchto premenných. Pri určovaní
dopadu môžeme brať do úvahy potenciálne finančné následky, priamy a latentný dopad na okolie alebo úsilie potrebne na nápravu a odstránenie
škody. Pri určovaní pravdepodobnosti môžem vychádzať z vlastných skúseností alebo štatistiky\footnote{Treba si však dať pozor, nakoľko
výsledky štatistiky sú závisle od kontextu a interpretácia jej výsledkov  môže byť subjektívna.}. Vo všeobecnosti na vyjadrenie rizika
používame dve metódy kvalitatívnu a kvantitatívnu, štandardy BSI využívajú tú prvú, no vysvetlíme oboje.
...
\textbf{Kvalitatívna metóda} vyjadruje hodnotu slovne, IT-Grundschutz používa $4 \times 4 \times 4$ úrovní, v poradí pravdepodobnosť
(\ref{tab:thrfreq}), dopadu (\ref{tab:thrdmg}) a riziko (\ref{tab:thrimp}). Prirodzene, v organizácii si počet a význam úrovní potrebujem
prispôsobiť svojím vlastným podmienkam a potrebám. 
\begin{table}[!h]
...
\begin{table}[!h]
    ...
    \begin{center}
        ...
        \hline
        vysoké & navrhnuté alebo implementované bezpečnostné opatrenia neposkytujú dostatočnú ochranu \\
        \hline
        veľmi vysoké & navrhnuté alebo implementované bezpečnostné opatrenia neposkytujú dostatočnú ochranu, v praxi sú málokedy akceptované \\
        \hline
        \end{tabularx}
    \end{center}
\end{table}

Nakoniec vzťah pravdepodobnosti, potenciálnej škody a potenciálneho dopadu vyjadríme maticou klasifikácie rizík \ref{fig:risk-matrix-bsi3}.
\input{tikz/risk-matrix-bsi3.tex}

\subsection*{Klasifikácia rizík: fiktívny scenár}
Vychádzajúc z fázy bezpečnostného procesu, kde sme definovali bezpečnostné požiadavky aktív, sme vytvorili tabuľku popisu hrozieb. Z predchádzajúcich
...
\input{table/risk_assessment.tex}
\newpage

\subsection{Ošetrenie rizík}
...
\textbf{Redukovať} riziko znamená vyvinúť a zaviesť efektívne opatrenia znižujúce pravdepodobnosť alebo dopad hrozby. Opatrenia môžeme vyvíjať
...
podmienky alebo ich zmeniť cez dodatky v prípade už uzavretých zmlúv.


\textbf{Akceptovať} riziko znamená zmieriť sa s potenciálnymi následkami naplnenia hrozby. Akceptované riziko následne monitorujeme ako zostatkové riziko.
V niektorých prípadoch je riziko prirodzenou súčasťou činnosti organizácie a jeho akceptovanie je súčasťou stratégie. Podobne existujú hrozby, voči ktorým
neexistujú efektívne protiopatrenia alebo náklady na jej minimalizovanie presahujú hodnotu aktíva.
...
\subsection*{Ošetrenie rizík: fiktívny scenár}
...
\input{table/risk_treatment}

\newpage

\subsection{Konsolidácia}
...
\begin{table}[h!]
    ...
    \begin{center}
            ...
            \textbf{narušenie schopnosti vykonávať úlohu}
             &
            narušenie by bolo posúdené ako znesiteľné, maximálny akceptovateľný prestoj je medzi 24 až 72 hodinami
             &
            narušenie by bolo posúdené ako neznesiteľné, maximálny akceptovateľný prestoj je medzi hodinou až 24 hodinami
             &
...
Poslednú praktickú radu, ktorú uvedieme, je sada piatich princípov, ktorými by sme sa mali riadiť pri určovaní
